...
exports.displayFiles = ( files = [] ) => {
    ...
    files.forEach( file => {
        ...
        itemDomElem.innerHTML = `
            <img ondragstart='copyFile(event, "${ file.name }")' src='../assets/document.png' class='app__files__item__file'/>
            ...
            <img onclick='deleteFile("${ file.name }")' src='../assets/delete.png' class='app__files__item__delete'/>
            <img onclick='openFile("${ file.name }")' src='../assets/open.png' class='app__files__item__open'/>
        `;
